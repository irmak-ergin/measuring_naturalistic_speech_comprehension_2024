...
p_mult <- ggplot(accuracy_results, aes(x = accuracy)) +
  ...
  labs(title = "All Participants' Multiple Choice Question Accuracy for Speech Rate x1",
       x = "Accuracy",
...
### Average word cound per second for each speech rate
```{r}
# per-trial word counts and words/min
df_wpm <- df.data %>%
  mutate(
    text_chr   = as.character(segment_text),
    word_count = if_else(
      is.na(text_chr) | stri_trim_both(text_chr) == "",
      0L,
      lengths(stri_extract_all_words(text_chr))
    ),
    wpm = if_else(duration > 0, 60 * word_count / duration, NA_real_)  # words per minute
  )

# summaries by speech_rate (both unweighted and duration-weighted)
wpm_by_rate <- df_wpm %>%
  group_by(speech_rate) %>%
  summarise(
    mean_wpm_unweighted = mean(wpm, na.rm = TRUE),
    sd_wpm_unweighted   = sd(wpm,   na.rm = TRUE),
    mean_wpm_weighted   = 60 * sum(word_count, na.rm = TRUE) / sum(duration, na.rm = TRUE),
    n_trials            = sum(!is.na(wpm)),
    .groups = "drop"
  ) %>%
  mutate(across(c(mean_wpm_unweighted, sd_wpm_unweighted, mean_wpm_weighted), ~ round(., 2)))
...
There is a main effect of Speech Rate for all 3, main effect of binning (recency) only for GloVe- Heard Summary & BERT.

## II. Validation
Can we predict the scores of our novel comprehension measure from the existing post-hoc comprehension measures?

### Correlation between comprehension measures

```{r}
vars <- c("trial_median_rescaled",
          "glove_sum_max_similarity_rescaled",
          "likert_rescaled",
          "multiple_choice_accuracy")

# <-- rename map to match your visreg wording
labs <- c(
  trial_median_rescaled              = "Real-Time Comprehension",
  glove_sum_max_similarity_rescaled  = "Semantic Similarity",
  likert_rescaled                    = "10-Point Scale",
  multiple_choice_accuracy           = "Multiple Choice"
)

X <- df.data[, vars]
rc_p <- rcorr(as.matrix(X), type = "pearson")

# apply pretty labels to matrices
R <- rc_p$r; P <- rc_p$P
dimnames(R) <- lapply(dimnames(R), function(nm) unname(labs[nm]))
dimnames(P) <- lapply(dimnames(P), function(nm) unname(labs[nm]))

# tidy pairwise table with pretty names
pairs_tbl <- (\(R, P) {
  nm <- colnames(R)
  out <- do.call(rbind, lapply(1:(ncol(R)-1), function(i)
    do.call(rbind, lapply((i+1):ncol(R), function(j)
      data.frame(var1 = nm[i], var2 = nm[j], r = R[i,j], p = P[i,j])
    ))
  ))
...
# palette: purple (-1) → white (0) → green (+1)
pal_pg <- colorRampPalette(c("#6A00A8", "#F7F7F7", "#10B981"))(200)
...
corrplot(R,
         ...
         insig     = "label_sig",   # prints ***, **, *
         pch.cex   = 1.2,
         pch.col   = "black")
```
```{r}
# with control measures too
...
vars <- c(
  ...
  "multiple_choice_accuracy",
  "digit_span_score",
  "digit_in_noise_score"
)

# 2) pretty labels (matching your visreg wording + controls)
labs <- c(
  trial_median_rescaled             = "Real-Time Comprehension Score",
  glove_sum_max_similarity_rescaled = "Semantic Similarity Score",
  likert_rescaled                   = "10-Point Scale Response",
  multiple_choice_accuracy          = "Multiple Choice Accuracy",
  digit_span_score                  = "Working Memory",
  digit_in_noise_score              = "Hearing In Noise"
)

# 3) data matrix (coerce any factor-like numeric columns)
X <- df.data[, vars]
X[] <- lapply(X, function(x) {
  if (is.factor(x)) suppressWarnings(as.numeric(as.character(x))) else x
})

# 4) correlations + p-values (Pearson)
...
# 5) palette: purple (-1) → white (0) → green (+1)
pal_pg <- colorRampPalette(c("#6A00A8", "#F7F7F7", "#10B981"))(200)

# 6) plot (upper triangle) with significance stars
corrplot(R,
         method    = "circle",
         type      = "upper",
         col       = pal_pg,
...
res_likert_round <- as.data.frame(res_likert$R2) %>%
...
Cohen's f2
```{r}
r2   <- r.squaredGLMM(model_likert)
R2_m <- as.numeric(r2[1])

# Cohen's f^2 for a single fixed effect
f2 <- if (is.finite(R2_m) && R2_m < 1) R2_m / (1 - R2_m) else NA_real_

# Partial R^2 equals Marginal R^2 when there's only one fixed effect
PR2_likert <- R2_m

# Pretty print (2 decimals)
cat(sprintf("Marginal R^2 (fixed effects): %.2f\n", R2_m))
cat(sprintf("Cohen's f^2: %.2f\n", f2))
cat(sprintf("Partial R^2 for likert_rescaled: %.2f\n", PR2_likert))
```
...
res_mcq <- partR2(model_multiple_choice, 
              ...
              R2_type = "marginal")
res_mcq
res_mcq_round <- as.data.frame(res_mcq$R2) %>%
  mutate(across(where(is.numeric), ~ round(., 2)))
res_mcq_round
```
Cohen's f2
...
res_sem <- partR2(model_glove1, 
              ...
              R2_type = "marginal")
res_sem
res_sem_round <- as.data.frame(res_sem$R2) %>%
  mutate(across(where(is.numeric), ~ round(., 2)))
res_sem_round
```
Cohen's f2
...
## Predicting speech rate from comprehension
...
## III. Multiple Choice Questions Diagnosis
Are questions comparable in difficulty?

Take all stories presented at 1x. (divide by questions and average across participants for that item)
```{r figure-size, fig.width=6, fig.height=6}

pid <- "participant"  

# 1) per-participant accuracy for each QUESTION at 1×
by_pp_1x <- df.data %>%
  filter(speech_rate == 1,
         !is.na(question), !is.na(.data[[pid]]),
         !is.na(multiple_choice_accuracy)) %>%
  group_by(.data[[pid]], question) %>%
  summarise(acc = mean(multiple_choice_accuracy, na.rm = TRUE), .groups = "drop")

# 2) per-question mean accuracy + n participants
q_summary_1x <- by_pp_1x %>%
  group_by(question) %>%
  summarise(
...
summary_1x <- q_summary_1x %>%
  summarise(
    n_questions = n(),
    mean  = mean(mean_acc), median = median(mean_acc),
    sd    = sd(mean_acc),   iqr    = IQR(mean_acc)
  )
print(round(summary_1x, 3))

#  histogram 
# counts of per-question means rounded to 2 decimals
counts_2dp <- q_summary_1x %>%
  mutate(mean_acc_2dp = round(mean_acc, 1)) %>%
  count(mean_acc_2dp) %>%
  arrange(mean_acc_2dp)

ggplot(counts_2dp,
       aes(x = factor(mean_acc_2dp, levels = sort(unique(mean_acc_2dp))), y = n)) +
  scale_x_discrete(expand = c(0,0)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, NA))+
  geom_col(width = 0.75, color = "white") +
  labs(x = "Accuracy", y = "Number of questions", title = "") +
...
tbl_1x <- df.data %>%
  filter(speech_rate == 1,
         !is.na(question), !is.na(.data[[pid]]),
         !is.na(multiple_choice_accuracy)) %>%
  # per-participant accuracy per question (in case of multiple trials)
  group_by(.data[[pid]], question) %>%
  summarise(acc = mean(multiple_choice_accuracy, na.rm = TRUE), .groups = "drop") %>%
  ...
  mutate(mean_accuracy = round(mean_accuracy, 2)) %>%   # 2 decimals
  arrange(desc(mean_accuracy))

tbl_1x
```
Caption: Distribution of per-question accuracy averaged across participants for speech rate x1

For 1 question about a segment, compare performance across Speech Rates (should lower as speeding up factor increases)
```{r}


# --- 1) Per-participant accuracy for each (question × speech_rate) ---
by_pp <- df.data %>%
  filter(!is.na(question), !is.na(speech_rate),
         !is.na(.data[[pid]]), !is.na(multiple_choice_accuracy)) %>%
  group_by(.data[[pid]], question, speech_rate) %>%
  summarise(acc = mean(multiple_choice_accuracy, na.rm = TRUE), .groups = "drop")

# --- 2) Average across participants for each (question × speech_rate) ---
agg <- by_pp %>%
  group_by(question, speech_rate) %>%
  summarise(
    mean_acc       = mean(acc, na.rm = TRUE),
...
# --- 3) Short question labels: Q1, Q2, ... ---
q_map <- df.data |>
  dplyr::filter(!is.na(question)) |>
  dplyr::distinct(question) |>
  dplyr::mutate(short_q = paste0("Q", dplyr::row_number()))

agg <- agg |>
  dplyr::left_join(q_map, by = "question")

# --- 4) Wide tables (averages and n’s), columns x1..x5, rounded to 2 decimals ---
# Averages
avg_wide <- agg |>
  dplyr::mutate(rate_lab = paste0("x", speech_rate)) |>
  dplyr::select(question, short_q, rate_lab, mean_acc) |>
  tidyr::pivot_wider(names_from = rate_lab, values_from = mean_acc) |>
  dplyr::mutate(dplyr::across(dplyr::starts_with("x"), ~ round(., 2))) |>
  dplyr::arrange(short_q)

# Counts
n_wide <- agg |>
  dplyr::mutate(rate_lab = paste0("x", speech_rate)) |>
  dplyr::select(question, short_q, rate_lab, n_participants) |>
  tidyr::pivot_wider(names_from = rate_lab, values_from = n_participants, names_prefix = "n_") |>
  dplyr::arrange(short_q)

avg_wide
n_wide

# --- 5) Multi-panel bar plot (Q1, Q2, ...), bars = x1..x5 ---
plot_df <- agg |>
  dplyr::mutate(rate_lab = factor(paste0("x", speech_rate),
                                  levels = paste0("x", sort(unique(speech_rate)))))

ggplot(plot_df, aes(x = rate_lab, y = mean_acc)) +
  geom_col(width = 0.75) +
  facet_wrap(~ short_q) +
  scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
  labs(x = "Speech rate", y = "Mean accuracy (across participants)") +
...
```
```{r}
# do it only for questions where you have at least 4 repsonses per sr

# ---- 0) Robust list of required rates (coerce in case it's a factor/char)
rates <- df.data$speech_rate
if (is.factor(rates)) rates <- as.numeric(as.character(rates))
if (is.character(rates)) rates <- as.numeric(rates)
required_rates <- sort(unique(rates))

# ---- 1) Per-participant accuracy per (question × speech_rate)
by_pp <- df.data %>%
  dplyr::filter(!is.na(question), !is.na(speech_rate),
                !is.na(.data[[pid]]), !is.na(multiple_choice_accuracy)) %>%
  dplyr::group_by(.data[[pid]], question, speech_rate) %>%
  dplyr::summarise(acc = mean(multiple_choice_accuracy, na.rm = TRUE), .groups = "drop")

# ---- 2) Coverage per (question × speech_rate): number of participants
coverage <- by_pp %>%
  dplyr::count(question, speech_rate, name = "n_participants") %>%
  dplyr::mutate(speech_rate = as.numeric(speech_rate)) %>%
  tidyr::complete(question, speech_rate = required_rates, fill = list(n_participants = 0))

# ---- 3) KEEP questions where every required rate has >= 4 participants
keep_q <- coverage %>%
  dplyr::group_by(question) %>%
  dplyr::summarise(min_n = min(n_participants),
                   n_rates = dplyr::n(), .groups = "drop") %>%
  dplyr::filter(min_n >= 3, n_rates == length(required_rates)) %>%
  dplyr::pull(question)

length(keep_q)  # how many questions pass the filter?

# ---- 4) Average across participants for kept questions
agg <- by_pp %>%
  dplyr::filter(question %in% keep_q) %>%
  dplyr::group_by(question, speech_rate) %>%
  dplyr::summarise(
    mean_acc       = mean(acc, na.rm = TRUE),
    n_participants = dplyr::n(),
    .groups = "drop"
...
# ---- 5) Short labels Q1, Q2, ... (only for kept questions)
q_map <- tibble::tibble(question = sort(unique(keep_q))) %>%
  dplyr::mutate(short_q = paste0("Q", dplyr::row_number()))

agg <- agg %>% dplyr::left_join(q_map, by = "question")

# ---- 6) Wide tables (averages and n’s), columns x1..x5, rounded to 2 decimals
avg_wide_filt <- agg %>%
  dplyr::mutate(rate_lab = paste0("x", speech_rate)) %>%
  dplyr::select(question, short_q, rate_lab, mean_acc) %>%
  tidyr::pivot_wider(names_from = rate_lab, values_from = mean_acc) %>%
...
avg_wide_filt
n_wide_filt
# write.csv(avg_wide_filt, "avg_accuracy_by_question_xrates_min4.csv", row.names = FALSE)

# ---- 7) Multi-panel bar plot (only kept questions), bars for x1..x5
plot_df <- agg %>%
  dplyr::mutate(rate_lab = factor(paste0("x", speech_rate),
                                  levels = paste0("x", required_rates)))
...
```
Do it only for the questions where average=1 for x1
...
# --- 2) Identify questions with perfect 1× average (across participants) ---
baseline_1x <- by_pp %>%
  dplyr::filter(speech_rate == 1) %>%
  dplyr::group_by(question) %>%
  dplyr::summarise(mean_acc_1x = mean(acc, na.rm = TRUE),
                   n_participants_1x = dplyr::n(), .groups = "drop")

# keep exactly-1; (use mean_acc_1x >= 0.999 if you want a tolerance)
keep_q <- baseline_1x %>%
  dplyr::filter(mean_acc_1x == 1) %>%
  dplyr::pull(question)

if (!length(keep_q)) stop("No questions have average 1× accuracy == 1 across participants.")

# --- 3) Average across participants for each (question × speech_rate), filtered ---
agg <- by_pp %>%
  dplyr::filter(question %in% keep_q) %>%
  dplyr::group_by(question, speech_rate) %>%
  dplyr::summarise(
...
# --- 4) Short question labels: Q1, Q2, ... (only for kept questions) ---
q_map <- tibble::tibble(question = sort(unique(keep_q))) %>%
  dplyr::mutate(short_q = paste0("Q", dplyr::row_number()))

agg <- agg %>% dplyr::left_join(q_map, by = "question")

# --- 5) Wide tables (averages and n’s) with columns x1..x5, rounded to 2 decimals ---
# Averages
avg_wide <- agg %>%
  dplyr::mutate(rate_lab = paste0("x", speech_rate)) %>%
  dplyr::select(question, short_q, rate_lab, mean_acc) %>%
...
# --- 6) Multi-panel bar plot for the filtered questions (bars = x1..x5) ---
plot_df <- agg %>%
  dplyr::mutate(rate_lab = factor(paste0("x", speech_rate),
                                  levels = paste0("x", sort(unique(speech_rate)))))
...
```
Ok this is more of an expected trend 
