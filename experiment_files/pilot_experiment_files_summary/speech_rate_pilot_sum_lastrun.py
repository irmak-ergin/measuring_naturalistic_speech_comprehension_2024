...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/gwilliamslab/Desktop/pilot_experiment_files_summary/speech_rate_pilot_sum_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    for thisTraining in training:
        ...
        summary_done.reset()
        # keep track of which components have finished
        SummaryComponents = [textbox, summary_done]
        ...
        training.addData('textbox.text',textbox.text)
        training.addData('summary_done.numClicks', summary_done.numClicks)
        if summary_done.numClicks:
           training.addData('summary_done.timesOn', summary_done.timesOn)
           training.addData('summary_done.timesOff', summary_done.timesOff)
        else:
           training.addData('summary_done.timesOn', "")
           training.addData('summary_done.timesOff', "")
    ...
    experiment = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('sub-0_shortened.xlsx'),
    ...
    for thisExperiment in experiment:
        ...
        for thisComponent in SummaryComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        thisExp.addData('Summary.stopped', globalClock.getTime())
        experiment.addData('textbox.text',textbox.text)
        experiment.addData('summary_done.numClicks', summary_done.numClicks)
        if summary_done.numClicks:
           experiment.addData('summary_done.timesOn', summary_done.timesOn)
           experiment.addData('summary_done.timesOff', summary_done.timesOff)
        else:
           experiment.addData('summary_done.timesOn', "")
           experiment.addData('summary_done.timesOff', "")
        # the Routine "Summary" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
