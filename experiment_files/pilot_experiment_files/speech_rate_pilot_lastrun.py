...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/gwilliamslab/Desktop/pilot_experiment_files/speech_rate_pilot_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    skip_button = visual.ButtonStim(win, 
        text="I didn't understand anything", font='Arvo',
        pos=(-0.5,-0.4),
        letterHeight=0.03,
        size=(0.5, 0.1), borderWidth=0.0,
        fillColor='darkgrey', borderColor=None,
    ...
    for thisTraining in training:
        ...
        for thisSum_true in sum_true:
            ...
            for thisDummy_loop in dummy_loop:
                ...
                textbox.reset()
                textbox.setText('')
                textbox.setPlaceholder('Type keywords here!')
                # keep track of which components have finished
                SummaryComponents = [countdown, textbox]
                ...
                while continueRoutine and routineTimer.getTime() < 15.0:
                    ...
                    if textbox.status == STARTED:
                        # is it time to stop? (based on global clock, using actual start)
                        if tThisFlipGlobal > textbox.tStartRefresh + 15-frameTolerance:
                            # keep track of stop time/frame for later
    ...
    experiment = data.TrialHandler(nReps=1.0, method='sequential', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('sub-3_shortened.xlsx'),
