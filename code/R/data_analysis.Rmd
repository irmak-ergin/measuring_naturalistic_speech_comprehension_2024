...
```
```{r}
# 1 way anova to report difference accuracy vtw speech rates
# regression too predicting s rate from multiple c accuracy 
```
...
```{r}
# comapare sem siilarity as fixed effect models to see which one explains the model better
# one good thing about glove is that it gives us a sense of which words explain better
# binnin to show it is wkm related (?)
```
